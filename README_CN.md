...
[ControlFlex](https://github.com/ControlFlexMC/control-flex)（CFX）的**社区模组兼容配置**仓库：通过 JSON 告诉 ControlFlex 如何正确处理第三方模组的按键、摇杆模式、跨层按键等。
...
将 `<实例名>` 替换为你的配置 / 实例名称。macOS、Linux 用 `/`；Windows 示例如下（也可用 `\`）。

| 启动器 | 典型路径 |
|--------|----------|
| **官方 Minecraft 启动器** | **Windows：** `%APPDATA%\.minecraft\config\controlflex\compat\`<br>**macOS：** `~/Library/Application Support/minecraft/config/controlflex/compat/`<br>**Linux：** `~/.minecraft/config/controlflex/compat/` |
| **Prism Launcher** / **Prism Launcher CE** / **MultiMC** / **PolyMC** | `<实例>/.minecraft/config/controlflex/compat/`<br>或 `<实例>/minecraft/config/controlflex/compat/`（以实际包含 `mods/` 的为准） |
| **CurseForge**（Overwolf） | **Windows：** `%USERPROFILE%\curseforge\minecraft\Instances\<实例名>\config\controlflex\compat\`<br>**macOS：** `~/Documents/curseforge/minecraft/Instances/<实例名>/config/controlflex/compat/`（因安装位置可能略有不同） |
| **Modrinth App** | **Windows：** `%APPDATA%\ModrinthApp\profiles\<实例名>\config\controlflex\compat\`<br>**macOS：** `~/Library/Application Support/ModrinthApp/profiles/<实例名>/config/controlflex/compat/`<br>**Linux（Flatpak）：** `~/.var/app/com.modrinth.theseus/data/ModrinthApp/profiles/<实例名>/config/controlflex/compat/` |
| **ATLauncher** | `<ATLauncher>/instances/<实例名>/config/controlflex/compat/` |
| **GDLauncher** / **Helios Launcher** | `<instances>/<实例名>/config/controlflex/compat/`（与 `mods/` 同级） |

若使用的启动器未列出，请在启动器中选择**打开实例目录**，查找是否已有 `config/controlflex/`（ControlFlex 运行过一次后会创建），或在 `mods/` 旁新建 `config/controlflex/compat/`。
...
| `loader` | string | 否 | `"forge"`、`"fabric"` 或 `"both"`（默认）。平台不符时跳过该配置。 |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `version` | string | 是 | 语义版本。`""` 表示匹配所有版本（兜底）。 |
...
1. **精确匹配** — `version` 与已安装模组版本相同。
2. **最近低版本** — 低于已安装版本的最高一条。
3. **兜底** — `"version": ""`。
